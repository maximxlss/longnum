...
void test_longnum_conversion() {
    ...
    assert_eq(LongNum(-123).to_string(), std::string("-123"));
}
...
void test_longnum_shifts() {
    ...
    assert_eq(x >> 7, "10011010101110101111100100000101001001101100001011000110.11000010111110000101000011010010010010010000011001010101010110001100110"_longnum);
    assert_eq(x >> 30, "100110101011101011111001000001010.0100110110000101100011011000010111110000101000011010010010010010000011001010101010110001100110"_longnum);
    assert_eq(-x << 70, "-1001101010111010111110010000010100100110110000101100011011000010111110000101000011010010010010010000011001010101010110001100110000000"_longnum);
    assert_eq(-x >> 7, "-10011010101110101111100100000101001001101100001011000110.11000010111110000101000011010010010010010000011001010101010110001100110"_longnum);
    assert_eq(-x >> 30, "-100110101011101011111001000001010.0100110110000101100011011000010111110000101000011010010010010010000011001010101010110001100110"_longnum);
    assert_eq(x << 123, x >> (-123));
...
void test_longnum_multiplication() {
    ...
    assert_eq(x.with_precision(256) * y, "60983583034582021399174027313270749532"_longdecimal);

    y = "111110001100111101010011101100010010110101101100000111"_longnum;

    x = "10110110010010011010100110000001.01100000000111001110101100101"_longnum;
    assert_eq(x.with_precision(114) * y, "10110001001010101111110101011101011011111111001111001101100111010000100010011000010101.10101101000110011010011000011"_longnum);

    y = "111011111100010101011100001000101101000.100111000101010111100000111"_longnum;
    assert_eq(x.with_precision(126) * y, "10101010101110110100110110001010011111001111011101110101010100010101010.00101111110110010011111001110011110110110101100011000011"_longnum);
}
...
void test_longnum_division() {
    ...
    assert_eq((1_longnum).with_precision(123) / 125_longnum, "0.00000010000011000100100110111010010111100011010100111111011111001110110110010001011010000111001010110000001000001100010010011011"_longnum);
    assert_eq((5_longnum).with_precision(128) / 13_longnum, "0.01100010011101100010011101100010011101100010011101100010011101100010011101100010011101100010011101100010011101100010011101100010"_longnum);
    assert_eq((123_longnum).with_precision(133) / 10_longnum, "1100.010011001100110011001100110011001100110011001100110011001100110011001100110011001100110011001100110011001100110011001100110011001"_longnum);
...
void test_longnum_utils() {
    ...
    assert_eq(LongNum(123).with_precision(128).pow(10), "792594609605189126649"_longdecimal);

    assert_eq(LongNum(123).to_int(), 123);
    assert_eq(LongNum(0).to_int(), 0);
    assert_eq(LongNum(-123).to_int(), -123);
    assert_eq("10101011010100100010101001011.1010100010101010011"_longnum.to_int(), 0b10101011010100100010101001011);
    assert_eq("101000010101001010011011010011.01100101"_longnum.with_precision(8).to_int(), 0b101000010000000000000000000000);
